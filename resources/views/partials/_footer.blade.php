<footer>
  <section class="infolinks">
    <div class="col-sm-3">
      <div class="logo">
        <img src="{{asset('img/logo.png')}}">
      </div>
      <div class="contact_info">
        <span><i class="glyphicon glyphicon-earphone"></i><strong>+91 80 4049 4077</strong></span>
        <span><i class="glyphicon glyphicon-envelope"></i><strong>services@shoppre.com</strong></span>
        <span><i class="glyphicon glyphicon-map-marker"></i><strong>Koramangala, Bengaluru</strong></span>
      </div>
    </div>
    <div class="col-sm-3">
      <ul class="foot_menu">
        <li><a href="{{route('home')}}">Home</a></li>
        <li><a href="{{route('about')}}">About Shoppre</a></li>
        <li><a href="/page/privacy-policy/">Privacy Policy</a></li>
        <li><a href="/page/terms-and-conditions">Terms & Conditions</a></li>
        <li><a href="/page/refund-and-cancellation-policy">Refund & Cancellation Policy</a>
        <li><a href="{{route('faq')}}">FAQ</a></li>
      </ul>
    </div>
    <div class="col-sm-3">
      <ul class="foot_menu">
        <li><a href="{{route('pricing')}}">Shipping Calculator</a></li>
        <li><a href="{{route('howitworks')}}">How Shoppre Works</a></li>
        <li><a href="{{route('contact')}}">Contact us</a></li>
        <li><a href="http://blog.shoppre.com/" target="_blank">Blogs</a></li>
        @if(Auth::guard('customer')->check())
          <li><a href="{{route('customer.logout')}}">Logout</a></li>
        @else
          <li><a href="{{route('customer.login')}}">Login</a></li>
          <li><a href="{{route('customer.register')}}">Sign up</a></li>
        @endif
      </ul>
    </div>
    <div class="col-sm-3">
      <div class="foot_social">
        <h3>Share the love!</h3>
        <a href="https://www.facebook.com/goshoppre/" target="_blank"><i class="fa fa-facebook-square"></i></a>
        <a href="https://twitter.com/Go_Shoppre" target="_blank"><i class="fa fa-twitter-square"></i></a>
        <a href="https://www.instagram.com/shoppre_official/?hl=en" target="_blank"><i class="fa fa-instagram"></i></a>
      </div>
    </div>
    <div class="clearfix"></div>
  </section>
  <section class="bottom">
    <div class="col-sm-6">
      <p class="copy">&copy; 2017, All Rights Reserved - by Shoppre</p>
    </div>
    <div class="col-sm-6">
      <div class="terms">
        <a href="#">Terms &amp; conditions</a>
        <span>|</span>
        <a href="#">Privacy policy</a>
      </div>
    </div>
    <div class="clearfix"></div>
  </section>
</footer>