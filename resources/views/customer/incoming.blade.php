		    ...
		    <div class="row">
			    <div class="col-sm-offset-2 col-sm-8">
			        <div class="incomings">
		                ...
		                @endif
		                <div class="title">
		                	<h2>Shoppre Incoming Orders</h2>
				        	<p> If you would like to alert us about any of your items that will be arriving at your locker in our facility, you can provide the details now!</p>
		                </div>
				        <form method="post" action="{{route('customer.incoming.submit')}}" enctype="multipart/form-data">
			                ...
			                <div class="form-group {{ $errors->has('track_no') ? 'has-error' : '' }}">
				                <label class="control-label"> Tracking Number(optional)</label>
			                    <input type="text" class="form-control" name="track_no" placeholder="Enter Tracking Number">
			                    @if ($errors->has('track_no'))
				                  <span class="help-block">
				                      <strong>{{ $errors->first('track_no') }}</strong>
				                  </span>
				                @endif
			                </div>
