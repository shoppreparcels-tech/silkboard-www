...
class PasswordController extends Controller
{
    ...
    public function submitForgot(Request $request)
    {
    	...
    	if (!empty($customer)) {
    		...
    		return redirect()->back()->with('error', 'Sorry, Email address submited not found!')->withInput($request->only('email'));
    	}
    }
    ...
    public function resetForgot(Request $request)
    {
    	if (isset($request->email)) {
    		$reset = PasswordReset::where('email', $request->email)
    							...
    							->first();
    		if(!empty($reset)){
    			return view('customer.reset')->with('email', $request->email);
    		}else{
