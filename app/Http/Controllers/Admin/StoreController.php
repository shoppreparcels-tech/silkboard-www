...
class StoreController extends Controller
{
    ...
    public function storesCategory(Request $request)
    {
        ...
        if (!empty($category)) {
            $stores = Store::all();
            $clubs = StoreCatClub::where('cat_id', $category->id)
                                    ->select('store_cat_clubs.*', 'stores.name', 'stores.type', 'stores.logo')
                                    ->join('stores', 'store_cat_clubs.store_id', '=', 'stores.id')
                                    ->get();
            return view('admin.categories-store')->with(['cat_id'=>$category->id, 'stores'=>$stores, 'clubs'=>$clubs ]);
        }else{
    ...
    public function updateStoreCat(Request $request)
    {
        ...
        $club = StoreCatClub::find($request->clubid);
