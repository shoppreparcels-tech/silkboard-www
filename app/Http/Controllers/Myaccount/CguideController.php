...
class CguideController extends Controller
{
	...
	public function viewCountry(Request $request)
	{
		...
		if (!empty($country)) {
			...
			$reviews = Review::where('countrid', $country->id)->where('approve', '1')->orderBy('updated_at', 'desc')->paginate(10);
			...
			return view('page.cguide.view')->with(['country' => $country, 'countries' => $countries, 'reviews' => $reviews, 'guide' => $guide, 'xchange_rate' => $xchange_rate]);
		}
