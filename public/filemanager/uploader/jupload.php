 ...
 * 			- full_name: the canonical file name (i.e. including the absolute path)
...
class JUpload {
	 ...
	 * relativePath, md5sum, mimetype, full_name, path
	 */
	...
	public function defaultAfterUploadManagement() {
		...
		foreach ($this->files as $f) {
			...
			foreach ($f as $key=>$value) {
				...
				if ($key != 'name' && $key != 'size' && $key != 'relativePath' && $key != 'full_name' && $key != 'md5sum') {
					if ($addBR) {
						$flist .= "<br>";
					} else {
...
if ($this->classparams['demo_mode']) {
	if ($jufinal || (!$jupart)) {
		...
		$files_data['full_name']	= 'Demo mode<BR>No file storing';
